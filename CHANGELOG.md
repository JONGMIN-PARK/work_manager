...
## v13.186 (2026-09-06) — 주간보고: 긴 업무 내용이 두 줄까지 표시

### 문제

항목 제목과 세부 줄 모두 `white-space:nowrap;text-overflow:ellipsis` 였다.
한 줄을 넘으면 길이에 상관없이 잘렸다 — `Laser Trimming System A1 변환 …` 처럼.

기한이 있는 항목은 v13.185 에서 메타 열을 줄여 여유가 생겼지만, **기한 없는 일반 업무**
(기타·C/S 등)는 메타 열 자체가 없어 원래 폭이 넉넉한데도 내용이 길어서 잘리고 있었다.
폭 조정으로는 해결되지 않는 경우다.

### 조치

한 줄 말줄임 → **두 줄까지 표시 후 말줄임**(`-webkit-line-clamp:2`).

| | 이전 | 이후 |
|---|---|---|
| 항목 제목 | `nowrap` + `ellipsis` (1줄) | `-webkit-box` + `line-clamp:2` |
| 세부 줄 | `nowrap` + `ellipsis` (1줄) | 〃 |
| 세부 줄 툴팁 | 없음 | `title` 추가 — 2줄도 넘치면 hover 로 전체 확인 |

- 대부분의 항목은 여전히 1줄로 끝나 v13.180 의 밀도형 레이아웃이 유지된다. 긴 것만 2줄로 펼쳐진다.
- 기한·진행률·D-day 는 `flex-shrink:0` 이라 2줄 행에서도 열 위치가 유지된다(오와열 정렬 유지).
- 항목 제목에 `line-height:1.35` 를 명시해 2줄일 때 행이 과하게 높아지지 않게 했다.
- `word-break:break-word` — 공백 없는 긴 영문/모델명이 칸을 뚫고 나가지 않게.

### 검증

Chromium — 1100·1280 비교 모드에서
`Laser Trimming System A1 변환 및 신규 사양 반영 작업 진행 중`(항목),
`Camera 제어 및 API 작업 — 길게 이어지는 …`(세부)이 2줄로 **전체 표시**,
짧은 항목 6건은 1줄 유지. 메타 3열 정렬 유지 확인.

### 영향

- 수정: `weekly-report-admin.js` (`renderItemRow` 제목, `renderDetail` 텍스트), `업무일지_분석기.html`(패치노트)
- **클라이언트 전용 — 서버 배포·마이그레이션 불필요.** 미리보기·비교 보기·HTML 내보내기에 동일 적용.

## v13.185 (2026-09-06) — 주간보고 제목 잘림 해소 + 타임라인 전체 폭 사용

### 1. 주간 업무 보고 — 우측 메타 3열 폭 축소

v13.184 로 제목을 굵게 하자 `SP3965-E…` 처럼 잘리는 게 눈에 띄었다. 원인은 굵기가 아니라
우측 메타 3열(기한·진행률·D-day)이 **실제 필요보다 넓게 고정**돼 있던 것.

| | 이전 | 이후 | 실측 필요폭 |
|---|---|---|---|
| 기한 | 48px | **40px** | `~09/15` 38px |
| 진행률 | 54px | **40px** | `100%` + 바 37px |
| D-day | 46px | **44px** | `D-365`/`D+123`/`D-DAY` 43px |
| 셀 간격 | 6px | **4px** | |
| **합계** | **160px** | **132px** | 제목에 **+28px** |

- 폭은 브라우저에서 실제 렌더 폭을 측정해 정했다(여유 2px). 처음에 36/38/42 로 잡았더니
  `~09/15`(38px 필요)가 40px 칸에 안 들어가 **두 줄로 접혀** 행 높이가 튀었다 — 측정 후 보정.
- 기한 칸에 `white-space:nowrap` 추가 — 폭이 모자라면 접히는 대신 잘리게 해서 오와열 정렬이 깨지지 않는다.
- 세 열 모두 고정폭 유지(v13.180 의 행 간 오와열 정렬 정책 그대로).

### 2. 프로젝트 타임라인 — 창 너비 그대로 사용

간트는 폭이 곧 정보량인데 전역 컨테이너 `.con{max-width:1400px}` 에 묶여 있었다.
간트 캔버스 자체는 `units.length * getUnitWidth()` 로 계산된 고정 폭(예: 224일 × 33px = 7444px)이라
컨테이너가 넓어질수록 **한 번에 보이는 날짜가 늘어난다.**

- `body.wide-mode` 클래스로 타임라인 탭에서만 `.con` 과 `.hdr-in` 의 1400px 캡을 해제.
  헤더를 같이 풀지 않으면 본문과 좌우가 어긋난다.
- `setMode()` 에서 대상 표시 직후 토글 — `_modeIsFresh()` 조기 return 앞이라 캐시 hit 으로 재진입해도 적용된다.
- `setPage()` 의 모드를 거치지 않는 페이지(사용자관리·주간보고·패치노트·메시지)에서는 해제.

### 검증

Chromium — 주간보고: 1280 비교 모드에서 기한·진행률·D-day 전부 한 줄(`~09/15`·`~11/30`·`D-9`·`D+36`),
메타/배지 넘침 0건, 제목 폭 A/B 194px → 230px(+36px, 보정 후 +28px).
타임라인: 1440/1920/2560 에서 `.con` 이 각각 1440/1920/2560px, 다른 탭 복귀 시 1400px 원복 확인.

### 영향

- 수정: `weekly-report-admin.js`(`META_W`, 기한 셀 nowrap), `업무일지_분석기.html`(`body.wide-mode` CSS, `_applyWideMode`, `setMode`/`setPage`)
- **클라이언트 전용 — 서버 배포·마이그레이션 불필요.**

## v13.184 (2026-09-06) — 주간 업무 보고: 기한 있는 항목 제목 굵게

기한이 박힌 항목은 제목을 굵게(`font-weight:700`) 표시한다. 날짜가 걸린 일과 그렇지 않은 일이
목록에서 한눈에 갈린다.

- 조건은 **`it.deadline` 존재** — 파서가 `~MM/DD` 를 항목에만 붙이므로(`weekly-report-parser.js:72`,
  `weekly-report-admin.js:720`) 세부 줄은 대상이 아니다. 진행률(`pct`)만 있는 항목도 대상이 아니다.
  "굵은 줄 = 일정이 있는 줄" 이라는 규칙이 흐려지지 않도록 기한 하나만 기준으로 삼았다.
- 굵기는 700 — 사이트 배지·섹션 라벨이 800 이라 같은 값을 쓰면 위계가 뭉개진다.
- **렌더 경로가 하나**라 한 곳만 고치면 전부 반영된다:
  `renderItemRow` ← `buildPreviewHTML` ← 작성 탭 미리보기 · 비교 보기 · `buildStandaloneHTML`(HTML 내보내기 라이트/다크).
  `web/`(Next.js) 주간보고 페이지는 D-day 를 별도 컬럼으로 두는 테이블이라 해당 없음.

### 검증

Chromium 으로 작성 탭에 4개 항목(기한+진행률 / 진행률만 / 기한+진행률 / 메타 없음)을 넣고 실측 —
`~09/15` 와 `~09/30` 이 붙은 두 항목만 `font-weight:700`, 나머지 둘은 `400` 유지.

### 영향

- 수정: `weekly-report-admin.js` (`renderItemRow`), `업무일지_분석기.html`(패치노트)
- **클라이언트 전용 — 서버 배포·마이그레이션 불필요.**

## v13.183 (2026-09-05) — 주간 분석 결과 표: 시간 줄바꿈 해소 + 수주명 폭 확대

### 문제

팀 관리 > 주간 분석 > 필터 결과 표에서 **"시간" 값이 두 줄로 접혔다.**

`style.css:14` 의 `*{box-sizing:border-box}` 때문에 컬럼의 `width` 는 패딩을 **포함**한 값이다.
`td{padding:9px 12px}` 이므로 좌우 여백만 24px — `<th style="width:40px">시간</th>` 은 숫자에 **16px** 만 남긴다.
`8.5`(12px JetBrains Mono 기준 약 22px)조차 들어가지 못해 줄바꿈됐다. 헤더 "시간"(10px + letter-spacing) 역시 46px 가 필요해 눌려 있었다.

같은 이유로 `업무분장`(65px) 헤더는 "업무분/장", `약자`(40px) 는 "약/자" 로 접히고 값은 `제어…` 로 잘렸다.

### 조치

| 컬럼 | 이전 | 이후 |
|---|---|---|
| 수주명 | 12% (≈161px) · 10px | **24%** (≈322px) · **11px** (본문과 동일) |
| 시간 | 40px | **78px** + `white-space:nowrap` |
| 업무분장 | 65px | **88px** |
| 약자 | 40px | **56px** |
| 표 최소 폭 | 700px | **1040px** |

- 시간 셀은 읽기 전용·편집모드 양쪽 모두 `white-space:nowrap` — 폭이 부족해도 접히는 대신 잘리게 해서 행 높이가 튀지 않는다.
- 짧은 헤더(이름·날짜·수주번호·업무분장·약자)에 `nowrap` 을 걸어 헤더가 2줄이 되는 일을 없앴다.
- 표 최소 폭이 늘어난 만큼 좁은 화면에서는 `.tw{overflow-x:auto}` 안에서 **표만** 가로 스크롤된다(페이지 자체는 밀리지 않음).

### 검증

Chromium 1440×900 — 컬럼 실측 `수주명 322px · 시간 78px · 업무분장 88px · 약자 56px · 업무내용 409px`,
`8.5 / 12.5 / 4.25 / 10` 전부 한 줄, 행 높이 41px(편집모드 39px), 헤더 전부 1줄.
390×844(모바일) — `document.scrollWidth == window.innerWidth`(페이지 가로 밀림 없음), 표 래퍼만 1040px 스크롤, 시간 셀 높이 35px 단일 행.

### 영향

- 수정: `업무일지_분석기.html` (결과 표 헤더 폭, 시간·수주명 셀 스타일, 패치노트)
- **클라이언트 전용 — 서버 배포·마이그레이션 불필요.** 새로고침으로 반영된다.
...
### 문제
1. **미리보기** — 기본 도형(○ 예정)이 **첫 세부 줄에만** 붙고 두 번째 줄부터는 도형 없이 `└` 로만 나왔다. `renderDetail`이 `di === 0` 일 때만 항목 상태를 반영했기 때문.
2. **파서** — 세부 줄에 흔히 쓰는 기본 도형(`-`, `·`, `•`, `○`, `*`, `└` 등)을 쓰면 그 줄이 **경고 없이 통째로 사라졌다**. `:` 와 `-.` 두 표기만 인식했기 때문.

### 조치
| 대상 | 이전 | 이후 |
|---|---|---|
| 세부 줄 도형 | 첫 줄만 상태 도형, 나머지 `└` | 무태그 세부 줄 **전부** 항목 상태(무상태=예정) 도형. 자체 `#완료`·`#진행중` 태그는 그대로 우선 |
| 하위 줄 | 구분 없음 | `└ ┗ ㄴ ↳ ➔ → ⇒` 시작 또는 4칸(탭 2회) 이상 들여쓰기 → `└` 마커 + 들여쓰기 |
| 세부 줄 인식 | `:` , `-.` | `:` , `-.` + `- – — * + · • ∙ ◦ ○ ● □ ■ ▪ ▶ >` 등 도형, 하위 도형, **도형 없이 들여쓴 줄** |
| 항목 줄 | `- [사이트] …` (대시+공백 필수) | 앞머리 도형 선택 — `○ [사이트]`, `[사이트]`, `-[사이트]` 모두 인식 |
| 섹션 오타 줄 | — | 알 수 없는 `[xxx]` 단독 줄을 항목으로 오인하지 않음 |

- 미리보기(`weekly-report-admin.js`)와 서버 파서(`server/services/weekly-report-parser.js`)에 **동일 규칙**을 적용 — 기존 parity 테스트로 강제된다.
- 들여쓰기 없는 맨텍스트 줄은 기존과 같이 무시한다(메모·주석 줄 오인 방지).

### 테스트
`test/weekly-report-bullets.test.js`(신규 7건) — 도형 16종 인식, 들여쓰기 규칙, 하위 줄 판정, 항목 줄 도형 변형, 섹션 오타 줄, 미리보기 도형/마커. `npm test` 총 21건 통과.

### 영향
- 수정: `weekly-report-admin.js`, `server/services/weekly-report-parser.js`, `업무일지_분석기.html`(패치노트), `test/weekly-report-bullets.test.js`(신규).
- **서버 변경 포함** — 배포 후 적용(신규 마이그레이션 없음). 저장된 보고서는 재업로드 시 누락됐던 세부 줄이 정상 적재된다.

## v13.180 (2026-08-09) — 주간 업무 보고 미리보기 개편 + 리팩토링

### 배경
주간업무 > 작성 탭의 지난주/금주 미리보기가 프로젝트명·담당자·기한·완료율·D-day가 흩어져 시인성이 낮고, 좁은 폭에서 이름이 과하게 잘렸다. "한눈에 팍 꽂히는" 보고서 형태로 재설계.

### 미리보기(작성 탭) 개편 — `weekly-report-admin.js`
- **밀도형 한 줄 레이아웃**: 1항목 = 1줄. `[사이트] 프로젝트명 · 기한 · 완료율 · D-day`, 세부는 압축 한 줄. 스크롤 없이 한 페이지에 담기도록 간격 축소.
- **상태 3색 아이콘**: 예정 = 파랑 빈 원 · 진행중 = 빨간 원 · 완료 = 녹색 체크. 3종 동일 크기(12px)로 통일, 작업 내용(세부) 줄 앞에 표시. 태그 없는 항목은 예정으로 자동 표시.
- **완료율 세로 히트바**: 폰트 높이 세로 바 + % 숫자, 구간별 색(빨강 <30 · 주황 <70 · 초록 ≥70)으로 대비 강화.
- **사이트 배지 폭 통일**: 전 섹션 최대 사이트명 기준으로 배지 폭을 맞춰 프로젝트명 시작 위치를 일괄 정렬. 세부 텍스트 시작 위치도 고정 슬롯으로 정렬.
- **우측 메타 오와열 정렬**: 날짜·진행률·D-day를 고정폭 3열로 두어 행 간 세로 열을 정렬(하나라도 있으면 3열 예약).
- **담당자 툴팁화**: 담당자 칩을 라인에서 제거하고 사이트 배지에 마우스 오버 시 `담당자: @a, @b` 툴팁으로 표시(공간 확보, 2~3명 대응).
- 업무 텍스트 폰트 통일(볼드 제거·타이틀 크기), 섹션 헤더의 완료/평균 요약 제거.
- **Pretendard 웹폰트** 적용(앱·내보내기) — 한글 시인성 향상. 폴백: Pretendard→Noto Sans KR→시스템 한글. 온라인/앱에서 로드, 이메일·오프라인은 폴백. 서버 CSP에 `cdn.jsdelivr.net`(style/font) 허용 추가.

### 리팩토링 (동작 무변경 — ④ 제외)
- **① 섹션 맵 단일화**: 4벌로 흩어진 섹션 정의(라벨/번호/색/별칭)를 `SECTIONS` 단일 소스로.
- **② 상수화**: 상태 색·아이콘 크기·완료율 히트 임계값·D-day 임계값을 config 상수로 추출.
- **③ 함수 분해**: 비대한 `buildPreviewHTML`을 헬퍼(statusIcon/iconSlot/pctBadge/renderDetail/renderSectionHeader/renderItemRow)로 분리 — 출력 바이트 동일 검증.
- **⑤ 테마 변수 이중관리 제거**: `:root` 생성·인라인 치환을 매핑 단일 소스에서 파생.
- **④ 파서 통일**: 클라이언트 파서를 서버(`server/services/weekly-report-parser.js`) 로직에 일치 — 태그 없는 "완료"도 완료로 판정. parity 테스트로 향후 drift 방지.
- **⑦ 테스트 추가**: `test/weekly-report.test.js`(node 내장 러너) 14종 — 파서·미리보기 불변식·클라이언트↔서버 parity. 루트 `package.json`에 `npm test`.

### 영향
- 수정: `weekly-report-admin.js`. 신규: `test/weekly-report.test.js`, `package.json`(루트).
- **동작 변화**: 미리보기에서 `#` 없는 "완료" 텍스트도 완료로 표시(서버·통계와 일관). 그 외 렌더 결과는 리팩토링 전과 동일.
- 서버/마이그레이션 변경 없음. 테스트: `npm test`(node --test) 14/14 통과.

## v13.179 (2026-08-01) — 요소기술 결함 수정 (진척률 리셋 · 화면 미갱신 · 검색 오탐)

### 수정된 결함

| # | 문제 | 원인 | 조치 |
|---|------|------|------|
| 1 | **진척률이 0%로 리셋** — 진척% 를 비운 채 문서·아이디어 일지를 쓰면 기술 진척률이 0이 됨 | 프론트가 빈 값을 `parseInt('') \|\| 0` → `0`으로 보내고, `resyncTech`가 최신 일지의 progress(0)를 그대로 반영 | 미입력은 **NULL**로 저장(`toProgress`). `resyncTech`는 `progress IS NOT NULL`인 최신 일지만 반영. 명시적 `0`은 그대로 존중 |
| 2 | 일지 등록·삭제 후 카탈로그 진척률·투입시간이 안 바뀜 | `_techList`만 갱신하고 재렌더 안 함 | `_techRefreshList()` — 재조회 + `_techRender()` |
| 3 | 검색 오탐 — "version"·"kind"로 검색하면 스택 보유 기술이 전부 매칭 | `t.stack::text ILIKE`가 JSON **키**까지 검색 | `jsonb_array_elements`로 스택 **name 값만** 검색 |
| 4 | 서버 `?stack=` 필터가 구현돼 있으나 미사용(죽은 코드) | 스택 보기 클릭이 클라이언트 `q` 검색으로 처리돼 부분일치 오탐 | 클릭 시 서버 `stack` 필터(정확 일치·대소문자 무시) 사용 + 적용 필터 배지·해제 |

### 영향
- 수정: `server/routes/tech.js`, `tech.js`, `업무일지_분석기.html`.
- **서버 변경 포함** — 배포 후 적용(신규 마이그레이션 없음).
- 기존 데이터 중 진척률 `0`으로 저장된 일지는 그대로 `0`으로 남습니다. 필요하면 해당 일지를 수정해 진척률을 다시 입력하면 재동기화됩니다.
...
### 배경
v13.174에서 사전검토 모듈을 급히 올리면서 남은 미완성·결함을 정리.

### 수정된 결함
| 문제 | 조치 |
|------|------|
| 담당자/참여자/태그가 DB·API엔 있으나 **UI가 없어 지정 불가** → `prestudy_assigned` 알림이 사실상 발동 못 함 | 모달에 담당자 select·참여자 체크박스·태그 입력 추가. 선택지는 신규 `GET /api/prestudies/members`(테넌트 활성 사용자 id·name만) |
| 노트에 "**굵게**·체크박스 지원" 안내만 있고 **서식 렌더가 없음** | `wmRichNote`(project-detail.js) 재사용한 「👁 미리보기」 토글 추가 |
| 전환 시 **브레인스토밍 노트 유실**(배경·결론만 복사) | 프로젝트·수주 메모에 notes 포함 |
| 회신기한이 캘린더에 **안 보임**(회의는 연동됨) | `048_prestudies_event.sql`(event_id) + `events(type='review')` 자동 발행/동기화. 기한 해제·검토 삭제 시 일정 정리 |
| 칸반이 드롭다운만 지원(앱의 다른 칸반은 드래그) | 드래그앤드롭 추가(드롭다운 병행 — 터치 환경 대비) |
| 담당자가 어디에도 표시 안 됨 | 목록 API에 `owner_name` JOIN, 칸반 카드·목록에 표시. 기한 초과 ⚠️ 강조 |

### 문서
- `docs/PRD_프로젝트관리_확장.md` — 폐기된 `/reviews`·`review_completed` 잔존 내용 제거, 일정 통합·알림·로드맵을 실제 구현에 맞게 정리.

### 영향
- 신규: `server/migrations/048_prestudies_event.sql`.
- 수정: `server/routes/prestudies.js`, `prestudy.js`, `project-data.js`, `업무일지_분석기.html`, `docs/PRD_프로젝트관리_확장.md`.
- **서버 변경 포함** — 배포 후 마이그레이션 자동 적용.
...
### 배경
v13.164에서 추가한 작업/할당 시간이 합계 한 줄이라, 누가 얼마나 할당받아 얼마나 했는지 담당자별로 보이지 않았음.

### 변경
- `_pdFillAllocInfo`를 합계 한 줄 → **담당자별 표**로 확장. 각 행 `실작업 / 할당 (달성률%)`, 맨 아래 합계. 할당 초과 시 빨간색.
  - 할당시간 = 마일스톤 목표시간(`assigneeTargets[이름]`), 실작업시간 = 작업노트 로그(`hours`)를 작성자(`authorName`)별로 합산.
  - 데이터: `msGetByProject`(할당) + `msLogsGet`(실작업) 병렬 조회.
- "투입시간 (h)" 라벨·설명을 두 줄로 정리하고, 담당자별 표를 입력칸 아래 별도 블록으로 배치.
